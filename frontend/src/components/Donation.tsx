...
const DonationSection: React.FC = () => {
  ...
  const openPopup = (method: 'paypal' | 'paystack') => {
  ...
  const handleItemMessageSubmit = async () => {
    try {
      ...
      if (response.ok) alert("Thank you! Your donation message was sent.");
  ...
  const handlePaystackPayment = () => {
    const publicKey =
      import.meta.env.VITE_PAYSTACK_PUBLIC_KEY || "pk_test_yourPublicKeyHere";
    ...
    paystack.newTransaction({
      ...
      metadata: {
        custom_fields: [
          { display_name: "Contact Info", variable_name: "contact_info", value: contactInfo },
        ],
      },
      onSuccess: async (transaction: { reference: string }) => {
        ...
        try {
          ...
          const redirectUrl =
            resp.ok && data.verified
              ? import.meta.env.VITE_SUCCESS_URL || "/payment-success"
              : import.meta.env.VITE_FAIL_URL || "/payment-failed";
          window.location.href = `${redirectUrl}?ref=${encodeURIComponent(transaction.reference)}`;
        } catch (err) {
          console.error("Verification request failed", err);
          window.location.href = import.meta.env.VITE_FAIL_URL || "/payment-failed";
        }
      },
      onCancel: () => {
          ...
          <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-xl overflow-hidden p-10 text-center">
            <div className='md:flex'>
                              ...
                              <PayPalButtons
                                ...
                                createOrder={(data, actions) => {
                                  return actions.order.create({
                                    intent: "CAPTURE",
                                    purchase_units: [{ amount: { value: donationAmount, currency_code: currency } }],
                                  });
                                }}
              ...
              <div className="md:w-1/2 bg-purple-50 p-8 md:p-12">
                ...
                <div onClick={() => handleItemClick("Food items")} className="bg-white p-4 rounded-lg shadow-sm mb-6 cursor-pointer hover:bg-gray-50 transition">
                  <div className="flex items-start">
                    <div className="bg-orange-100 p-3 rounded-full mr-4"><i className="fas fa-tint text-orange-500"></i></div>
                    <div>
                ...
                <div onClick={() => handleItemClick("School Supplies")} className="bg-white p-4 rounded-lg shadow-sm mb-6 cursor-pointer hover:bg-gray-50 transition">
                  <div className="flex items-start">
                    <div className="bg-orange-100 p-3 rounded-full mr-4"><i className="fas fa-book text-orange-500"></i></div>
                    <div>
                ...
                <div onClick={() => handleItemClick("Other items")} className="bg-white p-4 rounded-lg shadow-sm mb-6 cursor-pointer hover:bg-gray-50 transition">
                  <div className="flex items-start">
                    <div className="bg-orange-100 p-3 rounded-full mr-4"><i className="fas fa-heartbeat text-orange-500"></i></div>
                    <div>
